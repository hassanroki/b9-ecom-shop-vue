...
class BrandImageService
{
     ...
     * }
     */
    public function payloadForBrand(Brand $brand): array
    {
        $isUploaded = $this->isUploadedPath($brand->image);

        return [
            'image' => $this->resolveUrl($brand->image),
            'image_source' => $isUploaded ? 'upload' : 'url',
            'image_url' => $isUploaded ? '' : ($brand->image ?? ''),
        ];
    }
     ...
     */
    public function prepareForStorage(array $data, ?UploadedFile $uploadedFile = null, ?Brand $existing = null): array
    {
        ...
        if ($source === 'upload') {
            if ($uploadedFile !== null) {
                ...
                $data['image'] = $uploadedFile->store('brands', 'public');
    ...
    /**
     * Delete an uploaded brand image from storage when no longer needed.
     */
