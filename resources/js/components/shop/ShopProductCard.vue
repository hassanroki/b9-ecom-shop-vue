    ...
    <article
        ...
        <div class="relative aspect-square overflow-hidden bg-gray-100">
            ...
            <div class="absolute top-2 left-2 flex flex-col items-start gap-1.5">
                <span v-if="product.inStock"
                    class="rounded-full bg-green-50 px-2.5 py-1 text-xs font-medium text-green-700 shadow-sm">In
                    Stock</span>
            ...
            <button type="button" aria-label="Add to wishlist" :aria-pressed="isWishlisted"
                ...
                :class="{ 'text-red-600': isWishlisted }" @click="handleToggleWish">
        ...
        <div class="flex flex-1 flex-col p-3 md:p-4">
            ...
            <div class="mt-2 flex items-center gap-2">
                <span class="text-base font-semibold text-shop-primary-600 md:text-lg">{{ formatTaka(product.price)
                    }}</span>
                <span v-if="product.oldPrice" class="text-xs text-gray-400 line-through">{{ formatTaka(product.oldPrice)
                    }}</span>
            </div>
