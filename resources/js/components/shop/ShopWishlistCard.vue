...
<template>
    <article
        ...
        <div class="relative aspect-square overflow-hidden bg-gray-100">
            <Link :href="productShowUrl(item)" :aria-label="`View ${item.name}`" class="block h-full w-full">
        ...
        <div class="flex flex-1 flex-col p-3 md:p-4">
            ...
            <div class="mt-2 flex items-center gap-2">
                <span class="text-base font-semibold text-shop-primary-600 md:text-lg">{{ formatTaka(item.price)
                    }}</span>
                <span v-if="item.oldPrice" class="text-xs text-gray-400 line-through">{{ formatTaka(item.oldPrice)
                    }}</span>
            </div>
            <div class="mt-auto pt-3">
                <button v-if="item.inStock" type="button"
