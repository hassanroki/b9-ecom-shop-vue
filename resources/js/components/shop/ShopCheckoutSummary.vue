...
<template>
    <div class="rounded-xl border border-gray-200 bg-white">
        <button type="button" class="flex w-full items-center justify-between gap-2 p-5 text-left lg:cursor-default"
            @click="toggleSummary">
            <span class="text-lg font-semibold text-gray-900">Order Summary</span>
            <span class="flex items-center gap-2">
                <span class="rounded-full bg-shop-primary-50 px-2.5 py-0.5 text-xs font-medium text-shop-primary-700">
                    {{ itemCount }}
                    {{ itemCount === 1 ? 'item' : 'items' }}
                </span>
                <svg class="h-5 w-5 text-gray-400 lg:hidden" :class="{ 'rotate-180': isSummaryOpen }" fill="none"
        ...
        <div class="border-t border-gray-100 lg:block" :class="isSummaryOpen ? 'block' : 'hidden'">
            <ul class="divide-y divide-gray-100 px-5">
                <li v-if="isEmpty" class="py-8 text-center text-sm text-gray-400">
                    Your cart is empty.
                </li>
                <li v-for="item in items" v-else :key="item.productId" class="flex gap-3 py-4">
                    <div class="h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                        <img :src="imgUrl(item.img)" :alt="item.name" class="h-full w-full object-cover" />
                    </div>
                        ...
                        <div class="mt-2 flex items-center justify-between">
                            <div class="inline-flex items-center rounded-lg border border-gray-300">
                                <button type="button" :aria-label="`Decrease quantity of ${item.name}`"
                                    ...
                                    :disabled="item.qty <= 1" @click="emit('decrement', item.productId)">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                        stroke-width="2">
