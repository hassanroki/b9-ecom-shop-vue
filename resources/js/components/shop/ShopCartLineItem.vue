...
<template>
    <li class="grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-12 sm:items-center sm:gap-4">
        <div class="flex items-center gap-3 sm:col-span-6">
            <Link :href="productHref"
                ...
                <img :src="imgUrl" :alt="item.name" class="h-full w-full object-cover" />
            </Link>
            <div class="min-w-0">
                <Link :href="productHref"
        ...
        <div class="text-sm text-gray-600 sm:col-span-2 sm:text-center">
            <span class="text-gray-400 sm:hidden">Price: </span>{{ formatTaka(item.price) }}
        </div>
        <div class="sm:col-span-2 sm:flex sm:justify-center">
            <div class="inline-flex items-center rounded-lg border border-gray-300">
                ...
                <span class="w-10 text-center text-sm font-medium text-gray-900">{{ item.qty }}</span>
        ...
        <div class="text-sm font-semibold text-gray-900 sm:col-span-2 sm:text-right">
            <span class="text-gray-400 sm:hidden">Total: </span>{{ formatTaka(item.price * item.qty) }}
        </div>
    </li>
</template>
