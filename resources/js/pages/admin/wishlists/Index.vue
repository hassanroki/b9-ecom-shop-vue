...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <Heading title="Wishlists" description="View customer wishlist items across the store" />

        <form :action="index()" method="get"
            class="grid gap-4 rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border lg:grid-cols-[1fr_auto]">
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
            <div class="overflow-x-auto">
                <table class="w-full min-w-[1000px] text-sm">
                    ...
                    <tbody>
                        <tr v-for="wishlist in wishlists" :key="wishlist.id" class="border-b last:border-b-0">
                            <td class="px-4 py-3">
                                <p class="font-medium">
                                    {{ wishlist.user.name }}
                                </p>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="size-12 shrink-0 overflow-hidden rounded-md border bg-muted">
                                        <img v-if="wishlist.product.image" :src="wishlist.product.image"
                                            :alt="wishlist.product.name" class="size-full object-cover" />
                                    </div>
                                    <div>
                                        <p class="font-medium">
                                            {{ wishlist.product.name }}
                                        </p>
                                        <p v-if="!wishlist.product.is_active" class="text-muted-foreground">
                                            Inactive product
                                        </p>
                                    </div>
                                </div>
